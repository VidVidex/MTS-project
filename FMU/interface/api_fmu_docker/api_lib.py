...
class API_FMU_Docker:
    ...
    def send_update(self):
        try:
            logger.info(
                self.name + ": send update request to server (" + str(self.server) + ")"
            )
            ...
            print(
                self.name
                + ": error when trying to send update request to server ("
                + str(self.server)
                + ")"
            )
    ...
    def send_reset(self):
        try:
            logger.info(
                self.name + ": send reset request to server (" + str(self.server) + ")"
            )
            requests.get(self.server + "reset")
        except:
            print(
                self.name
                + ": error when trying to send reset request to server ("
                + str(self.server)
                + ")"
            )
    ...
    def set_input(self, input):
        ...
        try:
            logger.info(
                self.name
                + ": set new input values on server ("
                + str(self.server)
                + "): "
                + str(input)
            )
            ...
            print(self.name + ": error when trying to set new inputs on server.")
    ...
    def get_output(self):
        try:
            ...
            logger.info(
                self.name
                + ": get actual output values from server ("
                + str(self.server)
                + "): "
                + str(output)
            )
            ...
            print(
                self.name
                + ": unable to get output values from server ("
                + str(self.server)
                + ")"
            )
            return
    ...
    def get_output_names(self):
        try:
            logger.info(
                self.name
                + ": get configured output variable names from server ("
                + str(self.server)
                + "):"
                + str(config["output"])
            )
            ...
            print(
                self.name
                + ": error when trying to get configured output variable names from server ("
                + str(self.server)
                + ")"
            )
    ...
    def get_server_config(self, server):
        try:
            ...
            if config != "CONFIGERR":
                ...
                print(self.name + ": no configuration found on server.")
        except:
            print(self.name + ": unable to get configuration data.")
            return
    ...
    def get_all_variable_names(self):
        try:
            ...
            logger.info(
                self.name + ": get all available variable names from FMU model."
            )
            return varnames
        except:
            print(
                self.name
                + ": unable to get model variable names! Load configuration file."
            )
            sys.exit(1)
    ...
    def get_actual_inputs(self):
        try:
            ...
            print(self.name + ": unable to get actual set inputs of FMU model.")
    ...
    def get_config_inputs(self):
        try:
            ...
            logger.info(
                self.name
                + ": get set inputs as configured in configuration file from server."
            )
            return config["input"]
        except:
            print(self.name + ": unable to get configured inputs from server.")
            sys.exit(1)
    ...
    def get_timestep(self):
        try:
            ...
            print(
                self.name
                + ": unable to get set timestep information from server (simulation time steps)."
            )
            sys.exit(1)
    ...
    def init_interface_config_client(self, config_path):
        logger.info(
            self.name
            + ": try to load configuration file from client. config path: "
            + str(config_path)
        )
        try:
            ...
            logger.info(
                self.name
                + ": configuration file loaded and API interface is initiated."
            )
        except:
            print(self.name + ": error in loading configuration file.")
            sys.exit(1)

    def init_interface_config_server(self, server):
        logger.info(
            self.name
            + ": try to load configuration file from server ("
            + str(server)
            + ")"
        )
        try:
            ...
            logger.info(
                self.name
                + ": configuration file loaded from server and API interface is initiated."
            )
        except:
            print(
                self.name
                + ": error in loading configuration file from server ("
                + str(server)
                + ")"
            )
            sys.exit(1)
    ...
    def __init_api_config(self, config):
        ...
        try:
            ...
            print(self.name + ": specification error for FMU information.")
            logger.info(
                self.name
                + ": example: "
                + json.dumps({"fmu": "path to FMU model"}, indent=4)
            )
            sys.exit(1)
        ...
        try:
            ...
            print(self.name + ": specification error for server information.")
            logger.info(
                self.name
                + ": example: "
                + json.dumps({"server": "http://localhost:80/"}, indent=4)
            )
            sys.exit(1)
        ...
        try:
            ...
            logger.info(
                self.name + ": configured output: " + json.dumps(self.output, indent=4)
            )
        except:
            print(self.name + ": specification error for output information.")
            logger.info(
                self.name
                + ": example: "
                + json.dumps({"output": ["x", "y", "z"]}, indent=4)
            )
            sys.exit(1)
        ...
        try:
            ...
            logger.info(
                self.name
                + ": configured predefined input information: "
                + json.dumps(self.input_config, indent=4)
            )
        except:
            print(
                self.name + ": specification error for predefined input information."
            )
            logger.info(
                self.name
                + ": example: "
                + json.dumps(
                    {"input": [{"time": 10, "x_input": "ok", "y_input": "ok"}]},
                    indent=4,
                )
            )
            sys.exit(1)
        ...
        try:
            ...
            if timestep > 0.0:
                pass
            logger.info(
                self.name + ": configured timestep information: " + str(timestep)
            )
        except:
            print(self.name + ": specification error for timestep information.")
            logger.info(
                self.name + ": example: " + json.dumps({"timestep": 0.1}, indent=4)
            )
            sys.exit(1)
    ...
    def upload_config(self):
        ...
        try:
            ...
            logger.info(
                self.name + ": configuration file uploaded to server. " + str(r)
            )
        except:
            print(self.name + ": error when trying to upload config to server.")
    ...
    def upload_fmu(self, fmu_path=""):
        ...
        try:
            ...
            try:
                ...
                print(self.name + ": error when uploading FMU file to server.")
        except:
            print(self.name + ": error when uploading FMU file to server.")
    ...
    def download_fmu(self):
        try:
            ...
            print(self.name + ": unable to get FMU file from server.")
            return
